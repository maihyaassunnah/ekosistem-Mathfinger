...
function LoginForm() {
  ...
  // Success Loading Animation States
  const [isSuccessLoading, setIsSuccessLoading] = useState(false);
  const [successUserName, setSuccessUserName] = useState("");
  const [loginProgress, setLoginProgress] = useState(30);
  const [successPhaseText, setSuccessPhaseText] = useState("Memverifikasi kredensial akun...");
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.error) {
        ...
        setLoginProgress(40);
        setSuccessPhaseText("Kredensial valid. Menyiapkan hak akses...");
        ...
        setTimeout(() => {
          setLoginProgress(75);
          setSuccessPhaseText("Mempersiapkan dashboard Easy Learning House...");
        }, 350);
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-[#064e3b] via-[#070d1e] to-[#040814] dark:from-[#064e3b] dark:via-[#070d1e] dark:to-[#040814] flex flex-col justify-between items-center py-6 px-4 relative overflow-x-hidden">
        ...
        {tab === "google" ? (
          <div className="space-y-4 py-2 animate-in fade-in">
            ...
            >
              {isGoogleLoading ? (
                <>
                  <span className="inline-block w-4 h-4 border-2 border-slate-400 border-t-emerald-500 rounded-full animate-spin" />
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            >
              {isGoogleLoading ? (
                <>
                  <span className="inline-block w-3.5 h-3.5 border-2 border-slate-400 border-t-emerald-500 rounded-full animate-spin" />
                  <span>Menghubungkan ke Google...</span>
                </>
              ) : (
                <>
      ...
      {/* Fullscreen Professional Login Success Transition Overlay */}
      {isSuccessLoading && (
        <div className="fixed inset-0 z-[99999] flex flex-col items-center justify-center p-4 bg-[#070d1e]/90 backdrop-blur-xl animate-in fade-in duration-300">
          {/* Ambient Glow */}
          <div className="absolute w-80 sm:w-96 h-80 sm:h-96 rounded-full bg-emerald-500/20 blur-3xl pointer-events-none" />

          <div className="w-full max-w-sm sm:max-w-md bg-white/10 dark:bg-[#0f1a36]/90 border-2 border-emerald-500/40 rounded-3xl p-6 sm:p-8 text-center shadow-2xl relative z-10 backdrop-blur-md">
            {/* Animated Checkmark Badge */}
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-emerald-600 to-teal-400 text-white flex items-center justify-center mx-auto shadow-lg shadow-emerald-500/40 mb-4 animate-bounce">
            ...
            <div className="inline-flex items-center gap-1.5 px-3 py-0.5 mt-1.5 rounded-full bg-emerald-500/20 border border-emerald-400/30 text-emerald-300 text-xs font-extrabold uppercase tracking-wider">
              Easy Learning House
            </div>

            {/* Welcome User text */}
            <p className="text-sm font-bold text-slate-100 mt-3">
              Selamat datang kembali,{" "}
              <span className="text-emerald-400">{successUserName || "Pengguna"}</span>!
            </p>

            {/* Phase Subtitle */}
            <p className="text-xs text-slate-300 mt-1 min-h-[20px] transition-all">
              {successPhaseText}
            </p>
            ...
            <div className="mt-6 w-full">
              <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden relative shadow-inner">
                <div
                  className="h-full bg-gradient-to-r from-emerald-500 via-teal-400 to-emerald-300 rounded-full transition-all duration-300 ease-out shadow-[0_0_12px_rgba(16,185,129,0.8)]"
